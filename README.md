...
3. λ参数扫描(0.01~1.0)寻找最优平衡点
4. 边缘工控机离线推理验证+试运行闭环

**二期（飞书业务闭环）：**
5. 飞书消息机器人——橙/红预警Webhook推送交互卡片，值长厂长秒级接收
6. 飞书审批——工单审批流接入，AI自动生成→推送审批→状态回写MySQL
7. 飞书多维表格——预警/工单/健康度同步云端，管理层直接筛选穿透
8. 验收：预警提前量 ≥7 天, 零漏报, 误报率 <10%
